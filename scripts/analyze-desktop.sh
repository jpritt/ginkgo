...
home=/home-3/mpritt4@jhu.edu/work/jacob/ginkgo-desktop/ginkgo
dir=${home}/uploads/${1}
source ${dir}/config
distMet=$distMeth
...
then
  ...
  do
    ...
    then
      ...
      if [ -f ${dir}/${file}.cells ];
      then
        #echo ${home}/scripts/binUnsorted ${genome}/${binMeth} `wc -l < ${genome}/${binMeth}` <(zcat -cd ${dir}/${file}) `echo ${file} | awk -F ".bed" '{print $1}'` ${dir}/${file}_mapped `wc -l < ${dir}/${file}.cells` ${dir}/${file}.cells
...
####################################
# Repeat steps above for fine bins #
####################################
...
then
  ...
  do
    ...
    then
      ...
      if [ -f ${dir}/${file}.cells ];
      then
        ${home}/scripts/binUnsorted ${genome}/${binMethFine} `wc -l < ${genome}/${binMethFine}` <(zcat -cd ${dir}/${file}) `echo ${file} | awk -F ".bed" '{print $1}'` ${dir}/${file}_mapped_ `wc -l < ${dir}/${file}.cells` ${dir}/${file}.cells
      else
        ${home}/scripts/binUnsorted ${genome}/${binMethFine} `wc -l < ${genome}/${binMethFine}` <(zcat -cd ${dir}/${file}) `echo ${file} | awk -F ".bed" '{print $1}'` ${dir}/${file}_mapped 1
      fi
    ...
    # 
      ...
      if [ -f ${dir}/${file}.cells ];
      then
        ${home}/scripts/binUnsorted ${genome}/${binMethFine} `wc -l < ${genome}/${binMethFine}` ${dir}/${file} `echo ${file} | awk -F ".bed" '{print $1}'` ${dir}/${file}_mapped_ `wc -l < ${dir}/${file}.cells` {dir}/${file}.cells
      else
        ${home}/scripts/binUnsorted ${genome}/${binMethFine} `wc -l < ${genome}/${binMethFine}` ${dir}/${file} `echo ${file} | awk -F ".bed" '{print $1}'` ${dir}/${file}_mapped 1
      fi
      gzip ${dir}/${file}
    fi
  ...
  # Concatenate binned reads to central file  
  paste ${dir}/*_mapped* > ${dir}/dataFine
  rm -f ${dir}/*_mapped* ${dir}/*_binned*
...
if [ "$process" == "1" ]; then
  echo "Launching process.R $genome $dir $statFile data $segMeth $binMeth $clustMeth $distMet $color ${ref}_mapped $f $facs $sex $rmbadbins $maxploidy $minbinwidth $binMethFine dataFine"
  ${home}/scripts/process.R $genome $dir $statFile data $segMeth $binMeth $clustMeth $distMet $color ${ref}_mapped $f $facs $sex $rmbadbins $maxploidy $minbinwidth $binMethFine dataFine
fi
...
nbCols=$(awk '{ print NF; exit; }' $dir/SegCopyReclust)
for (( i=1; i<=$nbCols; i++ ));
do
  currCell=$(cut -f$i $dir/SegCopyReclust | head -n 1 | tr -d '"')
  if [ "$currCell" == "" ]; then
    continue;
  fi
  cut -f$i $dir/SegCopyReclust | tail -n+2 | awk '{if(NR==1) print "1,"$1; else print NR","prev"\n"NR","$1;prev=$1; }' > $dir/$currCell.cnv
done
