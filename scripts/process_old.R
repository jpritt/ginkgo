...
for(k in 1:w)
{
  ...
  jpeg(filename=paste(lab[k], "_dist", outSuffix, ".jpeg", sep=""), width=3000, height=750)
  ...
  jpeg(filename=paste(lab[k], "_counts", outSuffix, ".jpeg", sep=""), width=2500, height=1500)
  ...
  #Plot lorenz curves
  jpeg(filename=paste(lab[k], "_lorenz", outSuffix, ".jpeg", sep=""), width=2500, height=1500)
  ...
  jpeg(filename=paste(lab[k], "_hist", outSuffix, ".jpeg", sep=""), width=2500, height=1500)
  ...
  jpeg(filename=paste(lab[k], "_SoS", outSuffix, ".jpeg", sep=""), width=2500, height=1500)
  ...
  jpeg(filename=paste(lab[k], "_CN", outSuffix, ".jpeg", sep=""), width=3000, height=750)
...
if (clusterCells) {
  write.table(stats, file=paste(user_dir, "/SegStatsClust", sep=""), sep="\t", quote=FALSE)
} else {
...
if(cm == "NJ")
{
  ...
  write.tree(clust, file=paste(user_dir, "/clust", outSuffix, ".newick", sep=""))
} else {
  ...
  write(hc2Newick(clust), file=paste(user_dir, "/clust", outSuffix, ".newick", sep=""))
}
...
compare_CN <- function(cell1, cell2, thresh=30) {
    num_mismatches = 0
    CN1 <- final[,cell_ids[[cell1]]]
    CN2 <- final[,cell_ids[[cell2]]]
    num_bins = length(CN1)
    curr_mismatch_len = 0
    for (i in 1:num_bins) {
        if (CN1[i] == CN2[i]) {
...
score_cluster <- function(clust, threshold) {
  ...
  for (n in names(groups)) {
    ...
    for (i in 1:(clust_size-1)) {
      for (j in (i+1):clust_size) {
        mismatches = mismatches + compare_CN(curr_clust[i], curr_clust[j])
      }
    }
  }
  return(mismatches)
...
if (!clusterCells) {
  ...
  while (maxScore == 0) {
    ...
    maxScore = score_cluster(clust, maxThresh)
    if (maxThresh > 100)
      break
  }
...
###
command=paste("java -cp ", main_dir, "/forester_1025.jar org.forester.application.phyloxml_converter -f=nn ", user_dir, "/clust", outSuffix, ".newick ", user_dir, "/clust", outSuffix, ".xml", sep="");
...
if(cm == "NJ"){
  ...
  clust2 = hclust(d2, method = cm)
  clust2$labels = lab  
  write(hc2Newick(clust2), file=paste(user_dir, "/clust2", outSuffix, ".newick", sep=""))
}
...
if(cm == "NJ"){
  ...
  clust3 = hclust(d3, method = cm)
  clust3$labels = lab  
  write(hc2Newick(clust3), file=paste(user_dir, "/clust3", outSuffix, ".newick", sep=""))
}
