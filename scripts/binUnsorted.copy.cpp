...
int main(int argc, char *argv[]){
  ...
  if (cols == 2) {
    ...
    while (fscanf(bin_file, "%201s%i%i%i", &new_chr, &loc[high_bound], &mappable[high_bound], &good_count[high_bound]) != EOF)
    {
      if (strcmp(prev_chr, new_chr) != 0)
      {
        if (skip == false)
    ...
    for (int s=0; s < num_samples; s++) {
      ...
      for(int i=0; i < len; i++) {
        if (mappable[i]) {
          ...
          //if (i == 0 || !mappable[i-1])
          //  fprintf(outfile, "%f\n", (wgt * bins[s][i+1] / float(good_count[i+1])));
          //else if (i == (len-1) || !mappable[i+1])
          //  fprintf(outfile, "%f\n", (wgt * bins[s][i-1] / float(good_count[i-1])));
          //else
          //  fprintf(outfile, "%f\n", (wgt * bins[s][i-1] / (2*float(good_count[i-1])) + wgt * bins[s][i+1] / (2*float(good_count[i+1]))));
          fprintf(outfile, "%f\n", 0);
        }
        //fprintf(outfile, "%i\n", bins[s][i]);
      }
